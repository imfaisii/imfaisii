...
class BookingRepository extends BaseRepository
{
    ...
    public function getCustomerJobHistory(User $cuser): array
    {
        $jobs = $cuser
            ->jobs()
    ...
    public function getTranslatorJobHistory(User $cuser, int $pageNum): array
    {
        $jobs_ids = Job::getTranslatorJobsHistoric($cuser->id, 'historic', $pageNum)->pluck('id');
        $numpages = ceil(count($jobs_ids) / 15);
    ...
    public function getUsersJobsHistory(User $cuser, FormRequest $request)
    {
        $page = $request->get('page', "1");

        return $cuser && $cuser->is('customer')
            ? self::getCustomerJobHistory($cuser)
            : self::getTranslatorJobHistory($cuser, (int)$page);
    }

    public static function getErrorResponseAgainstKey(string $key)
    {
        return [
            'status' => 'fail',
    ...
    public function store($cuser, $data)
    {
        ...
        if ($cuser->user_type == RolesEnum::CUSTOM_ROLE_ID) {

            if (!isset($data['from_language_id'])) return self::getErrorResponseAgainstKey('from_language_id');

            if ($data['immediate'] == 'no') {
                if (isset($data['due_date']) && $data['due_date'] == '')
                    return self::getErrorResponseAgainstKey('due_data');

                if (isset($data['due_time']) && $data['due_time'] == '')
                    return self::getErrorResponseAgainstKey('from_language_id');

                if (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type']))
                    return self::getErrorResponseAgainstKey('customer_phone_type');

                if (isset($data['duration']) && $data['duration'] == '')
                    return self::getErrorResponseAgainstKey('duration');
            } else {
                if (isset($data['duration']) && $data['duration'] == '')
                    return self::getErrorResponseAgainstKey('duration');
            }
            ...
            // already lazy loaded so no extra queries
     ...
     * @return mixed
     */
    public function storeJobEmail(Job $job, array $data): array
    {
        // lazy loading user
        ...
        $this->mailer->send($job->user_email ?? $job->user->email, $job->user->name, $subject, 'emails.job-created', $send_data);

        $data = $this->jobToData($job);

        if ($job) Event::fire(new JobWasCreated($job, $data, '*'));
     ...
     * @return mixed
     */
    public function updateJob(Job $job, array $data, User $cuser): array
    {
        $current_translator =
            $job->translatorJobRel->whereNull('cancel_at')->first() ??
     ...
     * @return array
     */
    private function changeStatus(Job $job, array $data, Translator $changedTranslator): bool
    {
     ...
     * @return array
     */
    private function changeTranslator(Tranlator $current_translator, array $data, Job $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $current_translator->update(['cancel_at' => now()]);
                $log_data[] = [
                ...
                if ($data['translator_email'] != '') {
                    $data['translator'] = User::whereEmail('email', $data['translator_email'])->first()->id;
                }
    ...
    {
        // early return greatly improve the performance
        if ($old_due == $new_due) return false;
    ...
    public function acceptJob(Job $job, User $cuser): array
    {
    ...
    public function getPotentialJobs(User $cuser): array
    {
        ...
        // Removing irrelevant calls
        $userlanguage = UserLanguages::whereBelongsTo($cuser)->pluck('lang_id');
        $gender = $cuser->userMeta->gender;
        $translator_level = $cuser->userMeta->translator_level;
        $jobs = Job::getJobs($cuser->id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
